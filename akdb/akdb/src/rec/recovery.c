...
#include "recovery.h"
#define LOG_PATH(iniparser_getstring(AK_config,"general:log_path","./log"))
...
void AK_recover_operation(int sig) {
    ...
    char *latest = malloc(strlen(LOG_PATH)+strlen("/latest.txt")+1);
    strcpy(latest, LOG_PATH);
    ...
    char *destination = malloc(strlen(LOG_PATH)+strlen(filename)+2);
    strcpy(destination, LOG_PATH);
    strcat(destination, "/");
