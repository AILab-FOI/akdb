...
int insert_row_to_block(list *row_root, KK_block *temp_block)
{
	...
	{//insertig values of the list one by one
		...
		{ //if exist more headers
			...
			while(search_elem)
			{//going throught list of elements
				...
				{//go through row
					...
					{//delete and insert row
						...
						if((strcmp(some_element->attribute_name,temp_block->header[j%head].att_name)==0)
							&&(some_element->constraint==0))
							{//if exist new data then we must not copy old one
								exist_new_data=1;
							}
							some_element= (element) GetNextElement(some_element);
							if(some_element==0)
...
int delete_update_segment(list *row_root, int delete)
{
	...
	if(DEBUG)
		printf("\n table to delete_update from: %s, sorce %s", table, some_element->table);
