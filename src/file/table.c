 ...
 */
list_node *AK_get_column(int num, char *tblName) {
    AK_PRO;
    int num_attr = AK_num_attr(tblName);
 ...
 */
list_node *AK_get_row(int num, char * tblName) {
    AK_PRO;
    table_addresses *addresses = (table_addresses*) AK_get_table_addresses(tblName);
    list_node *row_root = (list_node *)AK_calloc(1, sizeof (list_node));
    Ak_Init_L3(&row_root);
 ...
 */
list_node *AK_get_tuple(int row, int column, char *tblName) {
    AK_PRO;
        ...
        for (j = addresses->address_from[ i ]; j < addresses->address_to[ i ]; j++) {
            ...
            for (k = 0; k < DATA_BLOCK_SIZE; k += num_attr) {
                ...
                if (counter == row) {
                    ...
                    AK_free(addresses);
                    AK_EPI;
                    return (list_node *) Ak_First_L2(row_root);
                }
            }
        }
        i++;
 ...
 */
char * AK_tuple_to_string(list_node *tuple) {
    int temp_int;
    float temp_float;
 ...
 */
void AK_print_row(int col_len[], list_node *row) {
    AK_PRO;
    list_node *el = (list_node *) Ak_First_L2(row);
    ...
    if ((addresses->address_from[0] == 0) || (AK_table_exist(tblName) == 0)) {
        ...
        for (i = 0; i < num_attr; i++) {
            for (j = 0; j < num_rows; j++) {
                list_node *el = AK_get_tuple(j, i, tblName);
                switch (el->type) {
                    case TYPE_INT:
        ...
        if (num_attr <= 0 || num_rows <= 0) {
            ...
            for (i = 0; i < num_rows; i++) {
                    list_node *row = AK_get_row(i, tblName);
 ...
 * @return No return value
 */
void AK_print_row_to_file(int col_len[], list_node * row) {
