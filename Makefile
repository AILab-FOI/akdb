...
pkgdatadir = $(datadir)/kalashnikov-db
pkglibdir = $(libdir)/kalashnikov-db
pkgincludedir = $(includedir)/kalashnikov-db
...
am__installdirs = "$(DESTDIR)$(kalashnikov_dbdocdir)"
kalashnikov_dbdocDATA_INSTALL = $(INSTALL_DATA)
DATA = $(kalashnikov_dbdoc_DATA)
...
PACKAGE_BUGREPORT = 
PACKAGE_NAME = kalashnikov-db
PACKAGE_STRING = kalashnikov-db 0.1
PACKAGE_TARNAME = kalashnikov-db
PACKAGE_VERSION = 0.1
PATH_SEPARATOR = :
SET_MAKE = 
SHELL = /bin/bash
...
top_srcdir = .
SUBDIRS = src 
kalashnikov_dbdocdir = ${prefix}/doc/kalashnikov-db
kalashnikov_dbdoc_DATA = \
	README\
	COPYING\
	AUTHORS\
	ChangeLog\
...
EXTRA_DIST = $(kalashnikov_dbdoc_DATA)
all: config.h
...
install-kalashnikov_dbdocDATA: $(kalashnikov_dbdoc_DATA)
	@$(NORMAL_INSTALL)
	test -z "$(kalashnikov_dbdocdir)" || $(MKDIR_P) "$(DESTDIR)$(kalashnikov_dbdocdir)"
	@list='$(kalashnikov_dbdoc_DATA)'; for p in $$list; do \
	  if test -f "$$p"; then d=; else d="$(srcdir)/"; fi; \
	  f=$(am__strip_dir) \
	  echo " $(kalashnikov_dbdocDATA_INSTALL) '$$d$$p' '$(DESTDIR)$(kalashnikov_dbdocdir)/$$f'"; \
	  $(kalashnikov_dbdocDATA_INSTALL) "$$d$$p" "$(DESTDIR)$(kalashnikov_dbdocdir)/$$f"; \
	done
...
uninstall-kalashnikov_dbdocDATA:
	...
	@list='$(kalashnikov_dbdoc_DATA)'; for p in $$list; do \
	  f=$(am__strip_dir) \
	  echo " rm -f '$(DESTDIR)$(kalashnikov_dbdocdir)/$$f'"; \
	  rm -f "$(DESTDIR)$(kalashnikov_dbdocdir)/$$f"; \
	done
...
installdirs-am:
	for dir in "$(DESTDIR)$(kalashnikov_dbdocdir)"; do \
...
install-data-am: install-kalashnikov_dbdocDATA
...
.PHONY: $(RECURSIVE_CLEAN_TARGETS) $(RECURSIVE_TARGETS) CTAGS GTAGS \
	...
	install-info-am install-kalashnikov_dbdocDATA install-man \
